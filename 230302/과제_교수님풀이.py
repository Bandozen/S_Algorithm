...
for tc in range(1, t+1):
    ...
    for i in range(len(arr)):
        ...
        for c in row:
            # 글자 하나 떼서 검사
            c_hex = int(c, 16)
            # 16진수는 이진수 * 4, 4번을 비교해준다.
            for j in range(3, -1, -1):
                bin_row += "1" if c_hex & (1 << j) else "0"

        # 이진수 문자열로 바꿨는데 그 안에 1이 없다??
        if "1" not in bin_row:
            # 다음 줄 검사
            continue
        else:
            ...
            # 모든 코드는 마지막이 1로 끝나니까 전체줄의 오른쪽 0은 모두 제거
            ...
            for j in range(len(bin_row) - 1, -1, -1):
                ...
                # 두번째 1의 개수 세기
                    ...
                    if len(new_code) == 8:
                        ...
                        new_code = []   # 한 줄에 다른 코드뭉치 있을수도 있으니 초기화
